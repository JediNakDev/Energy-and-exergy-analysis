...
# TODO: Edit PCMs' properties at 66 and 67th line
...
class PCM:
    def properties(self, p, c_ps, c_pl, L, emi, Tm, price):
        A = 1.61
        self.m = p*A*0.035
        self.c_ps = c_ps
        self.c_pl = c_pl
...
p1.properties(p=800, c_ps=2500, c_pl=2500, L=140000, Tm=296, emi=0.91, price=95.5696)
p2.properties(p=1460, c_ps=1631.65, c_pl=919.16, L=61630, Tm=305.55, emi=0.96, price=9.87252)
...
for q in ratio:
    r = q/100
    M1s2s = p1.M_s*r +p2.M_s*(1-r)
    M1l2s = p1.M_l*r +p2.M_s*(1-r)
    M1l2l = p1.M_l*r +p2.M_l*(1-r)
    M1sl = p1.M_sl*r
    M2sl = p2.M_sl*(1-r)
    print(M1s2s)

    p_emi = p1.emi*r +p2.emi*(1-r)
    ...
    while i < 380:
        ...
        if not chagestate:
            ...
            h_radep = (sigma * (Tp[2*i-1]**2 + Tie[2*i-1]**2)
                    * (Tp[2*i-1] + Tie[2*i-1]))/((1/e.emi)+(1/p_emi)-1)
            h_radpf = (sigma * (Tef[2*i-1]**2 + Tp[2*i-1]**2)
                    * (Tef[2*i-1] + Tp[2*i-1]))/((1/p_emi)+(1/f.emi)-1)
            h_radfa = sigma * f.emi * \
