"""
Temperature simulation for PV panel with single PCM layer.

TODO: Edit PCM's properties in the PCM configuration section.
...
# TODO: PCM configuration - change PCM type here
...
h_cong = H_CONV_GLASS
h_conf = H_CONV_BACK
state = 0  # PCM state: 0=solid, 1=liquid
...
# Helper Functions
# =============================================================================
def pop_all_temperatures():
    """Remove last two entries from all temperature arrays."""
...
def calculate_radiation_coefficients(idx):
    ...
    h['h_radep'] = (SIGMA * (Tp[idx]**2 + Tie[idx]**2) * (Tp[idx] + Tie[idx])) / ((1/e.emi) + (1/p.emi) - 1)
    h['h_radpf'] = (SIGMA * (Tef[idx]**2 + Tp[idx]**2) * (Tef[idx] + Tp[idx])) / ((1/p.emi) + (1/f.emi) - 1)
...
while i < 751:
    ...
    if not changestate:
        ...
        Tee.append(T[8][0]); Tie.append(T[9][0]); Tp.append(T[10][0])
        Tef.append(T[11][0]); Tif.append(T[12][0])
        
        Teg.append(T[13][0]); Tig.append(T[14][0]); Teb.append(T[15][0]); Tib.append(T[16][0])
        Tec.append(T[17][0]); Tic.append(T[18][0]); Ted.append(T[19][0]); Tid.append(T[20][0])
        Tee.append(T[21][0]); Tie.append(T[22][0]); Tp.append(T[23][0])
        Tef.append(T[24][0]); Tif.append(T[25][0])
...
if __name__ == "__main__":
    x = [datetime(2022, 3, 1, i // 30, (i % 30) * 2) for i in range(150, 570)]
